...
class r0926887:
    def __init__(self):
        ...
        # --- Memetic Algorithm (Hybrid) Parameters ---
        # local_search_rate: The probability of applying the 2-opt local
        # search to a newly created child.
        # - 1.0 = (Full Memetic Algorithm): Every child is optimized. Gives the
        #   best quality but is the slowest, especially for large 'n'.
        # - 0.1-0.3 = (Partial Memetic Algorithm): A good trade-off.
        #   Faster generations, still get the benefit of local search.
        # - 0.0 = (Standard Genetic Algorithm): No local search. Fastest,
        #   but solution quality will be much worse.
        self.local_search_rate = 0.25  # Set to 25% for a good speed/quality balance
    ...
    def length(self, tour, distance_matrix):
        """
        Calculate the total length of a TSP tour (cycle). O(n) operation.
        """
    ...
    def two_opt(self, tour, distance_matrix):
        ...
        *** NEW: This is the V4 *optimized* version. ***

        Instead of recalculating the entire tour length (O(n)) inside the
        loop, we now calculate the *change* in distance (delta) in O(1).
        This makes the inner part of the loop O(1) instead of O(n).

        The overall complexity is reduced from O(k * n^3) to O(k * n^2),
        which is a *massive* performance gain for large 'n' (like 250).
        """
        n = len(tour)
        improved = True
        best_tour = tour.copy()
        ...
        while improved:
            ...
            for i in range(n - 1):  # Iterate from 0 to n-2
                for j in range(i + 1, n):  # Iterate from i+1 to n-1
                    ...
                    # We are considering swapping the edges:
                    # (city i, city i+1) and (city j, city (j+1)%n)
                    # with:
                    # (city i, city j) and (city i+1, city (j+1)%n)

                    # This is done by reversing the segment from i+1 to j

                    # Get the four cities involved
                    city_i = best_tour[i]
                    city_i_plus_1 = best_tour[(i + 1) % n]
                    city_j = best_tour[j]
                    city_j_plus_1 = best_tour[(j + 1) % n]

                    # Calculate the distance of the *current* edges
                    current_dist = (distance_matrix[city_i, city_i_plus_1] +
                    ...
                    # Calculate the change in length (delta)
                    delta = new_dist - current_dist

                    if delta < 0:
                        # This swap is an improvement

                        # Reverse the segment from i+1 to j (inclusive)
                        # This is the standard 2-opt swap
                        ...
                        if start > end:
                            path_to_reverse = np.concatenate((best_tour[start:], best_tour[:end + 1]))
                            path_to_reverse = path_to_reverse[::-1]  # Reverse
                            best_tour[start:] = path_to_reverse[:n - start]
                            best_tour[:end + 1] = path_to_reverse[n - start:]
                        else:
                            best_tour[start:end + 1] = best_tour[start:end + 1][::-1]

                        best_length += delta  # Update length with O(1) delta
                        improved = True
    ...
    def optimize(self, filename):
        ...
        param_summary = (f"# Parameters: population_size={self.population_size}, "
                         ...
                         f"local_search_rate={self.local_search_rate}\n")
        ...
        while True:
            ...
            for _ in range(self.population_size):
                ...
                # --- NEW: Probabilistic Local Search ---
                if random.random() < self.local_search_rate:
                    # Apply the *optimized* 2-opt
